class AlertModel {
  ...
  static DateTime _parseDate(dynamic raw) {
    ...
    if (raw is String) {
      try {
        ...
        // ESP32 prototype sends 1970 placeholder; treat it as "now" so it appears in UI.
  ...
  static List<String> _parseComments(dynamic raw) {
    if (raw == null) return const [];
    if (raw is List) {
      return raw.map((e) => e?.toString() ?? '').where((s) => s.isNotEmpty).toList();
    }
    // In RTDB, comments are often stored as push keys: { "-N...": "text", ... }
    if (raw is Map) {
      final values = raw.values;
      return values.map((e) => e?.toString() ?? '').where((s) => s.isNotEmpty).toList();
    }
    return const [];
  }
